      ...
      <CardContent className="pt-6">
        <div className="relative flex justify-center mb-8">
          <svg className="w-32 h-32 -rotate-90">
            <circle cx="64" cy="64" r="56" stroke="rgb(229, 231, 235)" strokeWidth="16" fill="none" />
            {activities.map((activity, index) => {
              const previousPercentages = activities.slice(0, index).reduce((sum, act) => sum + act.percentage, 0)

              const circumference = 2 * Math.PI * 56
              const offset = (circumference * (100 - activity.percentage)) / 100
        ...
        <div className="space-y-2">
          {activities.map((activity) => (
            <div key={activity.name} className="flex items-center justify-between border-b-2 last:border-b-0 pb-1">
              ...
              <span className="text-[17px] text-muted-foreground">{activity.percentage}%</span>
            </div>
          ))}
        </div>
      </CardContent>
